...
import { LearningError } from '../domain/errors';
...
@Catch()
export class LearningExceptionFilter implements ExceptionFilter {
  catch(exception: unknown, host: ArgumentsHost): void {
    const domainError = exception instanceof LearningError ? exception : null;
    const httpError = exception instanceof HttpException ? exception : null;
    const status =
      domainError?.statusCode ?? httpError?.getStatus() ?? HttpStatus.INTERNAL_SERVER_ERROR;
    ...
    const body: ApiEnvelope<never> = {
      ...
      meta: {},
      error: {
        code: domainError?.code ?? (status === 400 ? 'VALIDATION_FAILED' : `HTTP_${status}`),
        message,
