...
export class OutboxRelayService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(@Inject(POSTGRES_POOL) pool: Pool) {
    ...
    this.publisher = new KafkaEventPublisher('learning-outbox', brokers);
    this.relay = new PgOutboxRelay(pool, 'learning', this.publisher);
  }
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      console.error('Learning outbox relay failed', error);
    } finally {
      this.running = false;
    }
  }
